...
HRESULT SoundManager::Init()
{
	...
	memset(channels, 0, sizeof(Channel*) * SOUNDBUFFER);
...
void SoundManager::Play(string key, float volume)
{
	...
	//map<string, Sound**>::iterator it = mapSoundData.find(key);
	//if (it == mapSoundData.end())
	//{
	//    return;
	//}

	//system->playSound(FMOD_CHANNEL_FREE, (it->second), false, channels[]
}
